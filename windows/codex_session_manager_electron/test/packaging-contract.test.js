...
test('Windows installer is a stable per-user NSIS package with login cleanup', () => {
  ...
  assert.match(configuration, /^appId: local\.codex\.session-manager$/m);
  ...
  assert.match(configuration, /CodexSessionKeeper-\$\{version\}-windows-\$\{arch\}-Setup/);
  assert.match(configuration, /UpdateKeys\.json/);
  assert.match(configuration, /UpdateServer\.json/);
  assert.match(configuration, /192\.168\.10\.54:18080\/codex-session-keeper\/stable\/windows/);
