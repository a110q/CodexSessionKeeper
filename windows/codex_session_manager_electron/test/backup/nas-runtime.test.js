...
test('update preparation drains the active NAS writer and can resume it after cancellation', async () => {
  const harness = runtimeHarness({ saved: configuration('运营部', '陈超') });
  await harness.runtime.initialize();

  assert.equal(await harness.runtime.stopAndDrain(5000), true);
  assert.deepEqual(harness.created[0].drainTimeouts, [5000]);
  harness.runtime.requestImmediateScan('wake');
  assert.deepEqual(harness.created[0].triggers, ['startup']);

  await harness.runtime.startPolling();
  assert.equal(harness.created.length, 2);
  assert.deepEqual(harness.created[1].pollingIntervals, [30000]);
  assert.deepEqual(harness.created[1].triggers, ['startup']);
});

test('failed update drain immediately resumes the existing NAS agent', async () => {
  const harness = runtimeHarness({
    saved: configuration('运营部', '陈超'),
    updateDrainResult: false,
  });
  await harness.runtime.initialize();

  assert.equal(await harness.runtime.stopAndDrain(5000), false);
  assert.deepEqual(harness.created[0].drainTimeouts, [5000]);
  assert.deepEqual(harness.created[0].pollingIntervals, [30000, 30000]);
});
  ...
  const runtime = createNasRuntime({
    ...
    agentFactory: ({ target: selectedTarget, paths, validateTarget, onProgress, onStatus }) => {
      const agent = {
        ...
        updateDrainResult: options.updateDrainResult !== false,
        drainTimeouts: [],
        pollingIntervals: [],
        triggers: [],
        report: onProgress,
        ...
        async stopAndDrain(timeout) {
          this.stopped = true;
          this.drainTimeouts.push(timeout);
          return this.updateDrainResult;
        },
