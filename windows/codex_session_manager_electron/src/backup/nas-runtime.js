...
function createNasRuntime({
  ...
  async function stopAndDrain(timeoutMs = 5000) {
    stopped = true;
    lifecycleGeneration += 1;
    invalidateScheduledRetry();
    const current = agent;
    ...
    const drained = typeof current.stopAndDrain === 'function'
      ? await current.stopAndDrain(timeoutMs)
      : await current.stopAndAwaitQuiescence(timeoutMs);
    if (!drained) {
      stopped = false;
      current.startPolling?.(pollInterval);
      return false;
    }
  ...
  function startPolling() {
    stopped = false;
    if (agent) {
      agent.startPolling?.(pollInterval);
